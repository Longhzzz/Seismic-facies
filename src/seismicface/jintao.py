...
def jintao_data(name,window,step,norm,shift,ep=2,property='sx',train_mode='ssl',train_dataset='train',ext=1,t=1):
    ...
    if os.path.exists(path+"_"+property+"_ssldata_window_"+str(window)+"step_"+str(step)+".dat"):
        ...
        if norm:
            sx = xnorm(sx,drop=True,upper=99,floor=1)
        horp = path + "p10.dat"
                    ...
                    if i2 - ext > 0 and i2 + ext < n2:
                        ...
                        for i21 in range(-ext,ext+1,t):
                            for i22 in range(-ext,ext+1,t):
                                ...
                                xx = i2 + i22
                                h = round(hor[ix,xx])
                                s.append(sx[ix][xx][h-window+shift:h+window+1+shift].tolist())#+[inline_zs[ix],xline_zs[xx]]
                        l.append(s)
                        p.append([2*i3,i2,round(hor[i3,i2]),i3,i2])
    ...
    for n,i in enumerate(points):
        ...
        train_label_label.append([2*inline,xline,labels[n],inline,xline])
